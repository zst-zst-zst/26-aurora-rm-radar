...
SDK_LOG_PATH=`cat ${LOGSERVER_PATH}/Path.ini`
USER_ID=`id -u`
...
if [ "Debian" = "$OS" ]; then
	...
	sed -i "/^SDK_HOME=*/s/${FIND_PATH}/${REPLACE_PATH}/" ${LOGSERVER_PATH}/Kylin/MvLogServerd
	cp -f ${LOGSERVER_PATH}/Kylin/MvLogServerd /etc/init.d/
elif [ "OpenEuler" = "$OS" ]; then
	sed -i "/^SDK_HOME=*/s/${FIND_PATH}/${REPLACE_PATH}/" ${LOGSERVER_PATH}/OpenEuler/MvLogServerd
	cp -f ${LOGSERVER_PATH}/OpenEuler/MvLogServerd /etc/init.d/
else
	sed -i "/^SDK_HOME=*/s/${FIND_PATH}/${REPLACE_PATH}/" ${LOGSERVER_PATH}/Debian/MvLogServerd
	cp -f ${LOGSERVER_PATH}/Debian/MvLogServerd /etc/init.d/
fi
...
if [ "Debian" = "$OS" ]; then
    ...
    systemctl enable MvLogServer
    systemctl start MvLogServer
else
...
cp -f LogServer.ini ${SDK_LOG_PATH}/MvSdkLog/