...
int MV_AddDevice(PSERIAL_DEVICE pstSerialDevice)
{
    ...
    nStatus = alloc_chrdev_region(&pstSerialDevice->nDevMajor, 0, 1, (const char *)pstSerialDevice->chVirSerialDevName);
    if (nStatus < 0) {
...
#else
void VirSerial_ModuleRemove(struct platform_device * pstPlatformDevice)
#endif
{
    int                             nRet = MV_OK;
    PSERIAL_DEVICE                  pstSerialDevice = NULL;
...
#else
    pstSerialDev->pTxBuffer = port->state->port.xmit_buf;
    tx_count = kfifo_len(&port->state->port.xmit_fifo);
...
extern int virserial_read_char(PSERIAL_DEVICE pstSerialDev, char chData)
{
    ...
    nRet = tty_insert_flip_char(pstTtyport, chData, TTY_NORMAL);
...
int MV_TermiosFromUser(unsigned long nArg, PMV_KTERMIOS pstKtermios)
{
    ...
    return copy_from_user( pstKtermios->pKtermios, (void*)nArg, sizeof(struct termios));
}
...
int MV_TermiosToUser(unsigned long nArg, void* pKtermios)
{
    ...
    return copy_to_user((void*)nArg, pKtermios, sizeof(struct termios));
}
