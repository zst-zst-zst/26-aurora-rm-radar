...
int MV_LockIrqSaveAcquire(MV_LOCK* aLock, OS_LOCK_FLAG nFlags)
{
    spinlock_t* lSpinlock = NULL;
    if(NULL == aLock)
    {
    ...
    spin_lock_irqsave( lSpinlock, nFlags);
...
int MV_LockIrqRestoreRelease(MV_LOCK* aLock, OS_LOCK_FLAG nFlags)
{
    spinlock_t* lSpinlock = NULL;
    ...
    spin_unlock_irqrestore( lSpinlock, nFlags);
