...
// 驱动支持的设备
struct pci_device_id LcDeviceId[] = {
    {LC_VENDOR_ID, LC_DEVICE_ID, LC_VENDOR_ID, LC_SUBSYSTEM_ID, 0, 0, 0},
    {0,}
};

MODULE_DEVICE_TABLE(pci, LcDeviceId);

struct pci_driver LcDriver = {
...
struct file_operations lc_fops = {
    .owner          = THIS_MODULE,
    .open           = ModuleOpen,
...
int MV_AddDevice(void* pPciDevice, char* chDeviceName, char* chLcClassName)
{
    ...
    cdev_init(&pstPciDevice->stCdev, &lc_fops);
    // 将cdev和设备号关联起来
    ...
    pstPciDevice->pstClass = class_create(THIS_MODULE, (const char *)chLcClassName);
#else
	pstPciDevice->pstClass = class_create((const char *)chLcClassName);
#endif
...
static int InitModule(void)
{
    ...
    printk(KERN_INFO"[%s:%d]:LC Driver Enter: Version[%s]\n", __FUNCTION__, __LINE__, MV_VERSION);

    // 注册pci driver
    nRet = pci_register_driver(&LcDriver);
    if (nRet < 0) {
...
static void ExitModule(void)  
{
    ...
    pci_unregister_driver(&LcDriver);
    printk(KERN_INFO"[%s:%d]:end\n", __FUNCTION__, __LINE__);
}
