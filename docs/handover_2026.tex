...
\toprule
局次 & 开始时间 & 统计时长 & 平均发送车辆数 & 发送 $\ge$3 辆时间占比 & 标记状态跳变 & 双倍易伤激活 \\
\midrule
...
（含局外时段）；$^{**}$第九局局内无车辆发送数据（雷达基本未工作），标记状态跳变疑为
对局残留帧所致，不作有效计数。}
\end{table}
...
\textbf{（3）}上述两条导致发送车辆数偏低且波动大：第五、六局局内平均每帧发送
3.48/3.47 辆（发送 $\ge$3 辆时间占比 97\%/95\%），第七、八局降至 2.39/2.21 辆
（52\%/42\%），第九局雷达基本未发送车辆数据；
