...
（录制时长含局间与赛前赛后时段，故大于 7 分钟；另有 5 月 21 日 10:02 的 75 秒短包
与两个无法解析的损坏包未列入统计）。局均易伤约 475 秒/局。
...
\toprule
局次 & 开始时间 & 录制时长 & 平均发车数 & 发车 $\ge$3 辆占比 & 标记状态跳变 & 双倍易伤激活 \\
\midrule
...
第五局 & 5.22 19:51 & 503s & 3.15 & 83\% & 30 & 0s \\
第六局 & 5.22 19:59 & 4969s & 2.75 & 60\% & 41 & 0s \\
第七局 & 5.23 15:53 & 586s & 1.85 & 38\% & 26 & \textbf{30s} \\
第八局 & 5.23 16:04 & 712s & 1.36 & 24\% & 27 & \textbf{30s} \\
第九局 & 5.24 09:24 & 627s & 0.09 & 0\% & 35$^{*}$ & 0s \\
\bottomrule
\end{tabularx}
\caption{区域赛全部 9 局回放统计。$^{*}$第九局雷达基本未发车，标记状态跳变
疑为对局残留帧所致，不作有效计数。}
\end{table}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.98\textwidth]{figs/vuln_stats.png}
  \caption{各局平均发送车辆数、发送 $\ge$3 辆时间占比与双倍易伤激活时长
  （依据回放统计结果绘制）。}
\end{figure}
...
\textbf{（3）}上述两条导致发车数长期偏低（前四局平均 1.97$\sim$3.23 辆，
其中第二局约九成时间不超过 3 辆；第九局雷达基本未发车）；
\textbf{（4）}已发送坐标精度不足，偏差 $\ge$1.6m 按规则倒扣分数；
\textbf{（5）}第一、二局标记状态全程无跳变，机会数恒为 0，双倍易伤无法触发；
\textbf{（6）}第五局起标记状态出现 26$\sim$41 次跳变、机会数达到 2 次，
但 9 局中仅第七、八局各激活一次 30 秒（每局上限 2 次共 60 秒，利用率约 11\%）。
